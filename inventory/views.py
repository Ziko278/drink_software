            ...
            try:
                with transaction.atomic():
                    ...
                    if stock_in_summary.status == 'confirmed':
                        ...
                        if empty > stock_in_summary.total_quantity:
                            supplier.balance += (empty - stock_in_summary.total_quantity) * site_setting.price_for_empty
                        else:
                            supplier.balance -= (stock_in_summary.total_quantity - empty) * site_setting.price_for_empty
                        ...
                        shortfall_or_surplus = empty - stock_in_summary.total_quantity
...
def process_pending_stock_in_view(request, pk):
    ...
    validate category capacities, update selling prices, deduct payment from supplier,
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                # STEP 1: Calculate total cost of stock-in items
                total_cost = sum([Decimal(str(item.total_cost_price)) for item in stock_in_items])
                ...
                # Convert empty and category.number_of_empty to Decimal
                empty = Decimal(str(stock_in_summary.empty or '0'))
                sold_category.number_of_empty = Decimal(str(sold_category.number_of_empty or '0'))

                if empty > sold_category.number_of_empty:
                    raise ValueError(
                        f"Stock-in empty containers ({empty}) exceed current category empty containers ({sold_category.number_of_empty})."
                    )

                sold_category.number_of_empty -= empty
                sold_category.save(update_fields=['number_of_empty'])

                # Convert total_quantity and price_for_empty to Decimal
                total_quantity = Decimal(str(stock_in_summary.total_quantity or '0'))
                ...
                if empty > total_quantity:
                    supplier_balance += (empty - total_quantity) * price_for_empty
                else:
                    supplier_balance -= (total_quantity - empty) * price_for_empty
                ...
                shortfall_or_surplus = empty - total_quantity
                total_empty_adjustment = abs(shortfall_or_surplus) * price_for_empty
                final_balance = f"₦{supplier.balance:,.2f}"
                supplier_name = escape(supplier.name)
                category_name = supplier.category.name.upper() if supplier.category else 'N/A'
